...
public static class PlaceholderGenerator
{
    ...
    {
        { ElementType.Fire, Color.FromArgb(255, 80, 30) },
        { ElementType.Water, Color.FromArgb(30, 100, 200) },
        { ElementType.Earth, Color.FromArgb(100, 70, 40) },
        { ElementType.Air, Color.FromArgb(180, 220, 255) },
        { ElementType.Light, Color.FromArgb(255, 240, 180) },
        { ElementType.Dark, Color.FromArgb(40, 20, 50) },
        { ElementType.Arcane, Color.FromArgb(150, 50, 200) },
        { ElementType.Nature, Color.FromArgb(60, 140, 60) }
    };
    ...
    public static void GenerateAllPlaceholders(string outputDirectory)
    {
        ...
        // Generate type-only placeholders (generic element - Fire)
        foreach (var type in cardTypes)
        {
            using var bitmap = GenerateCardPlaceholder(ElementType.Fire, type, type.ToString());
            var filename = Path.Combine(outputDirectory, $"{type}_fire.png");
            bitmap.Save(filename, ImageFormat.Png);
            generated++;
        }
